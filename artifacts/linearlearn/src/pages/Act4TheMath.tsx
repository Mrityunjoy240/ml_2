...
import { mean, mse, normalEquation } from "@/lib/math";
import {
  ScatterChart,
  Scatter,
  XAxis,
...
export default function Act4TheMath() {
  const { dataset, targetColumn, featureColumns, addGlossaryTerm, worksheetScores, setWorksheetScore } = useAppState();
  ...
  const handleOptimize = () => {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      history.push({ b0: currB0, b1: currB1, loss: mse(targetVals, preds) });
    }
  ...
  if (step >= 2) {
    addGlossaryTerm({ term: "Intercept (b0)", definition: "The starting value. If the feature was 0, what would the prediction be?" });
    addGlossaryTerm({ term: "Slope (b1)", definition: "The multiplier. For every 1 unit increase in the feature, how much does the prediction change?" });
    ...
    <div className="flex flex-col gap-12 pb-32">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
        ...
        <p className="text-xl text-muted-foreground leading-relaxed max-w-2xl">
          It is time to build the formula. We need the best starting point and the best multiplier for the line.
        </p>
      </motion.div>
              ...
              <div className="h-[400px] bg-card border border-border p-4 rounded-xl shadow-lg relative">
                <ResponsiveContainer width="100%" height="100%">
                  <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                    ...
                    />
                    {step >= 2 &&
                      scatterData.slice(0, 20).map((pt, i) => {
            ...
            <div className="space-y-8">
              {step === 1 && (
                <div className="space-y-6">
                  ...
                  <div className="bg-muted/30 p-6 rounded-xl font-mono text-xl border border-border text-center">
                    y = <span className="text-accent">b0</span> + <span className="text-primary">b1</span>x
                  </div>
                  <p className="text-muted-foreground">Every straight line is defined by two numbers: the starting point (b0) and the slope (b1).</p>

                  <div className="space-y-8 pt-4">
                    <div className="space-y-4">
                      <div className="flex justify-between">
                        <label className="font-medium text-accent">b0 (Starting point)</label>
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between">
                        <label className="font-medium text-primary">b1 (Multiplier)</label>
              ...
              {step === 2 && (
                <div className="space-y-6">
                  ...
                  <p className="text-muted-foreground">
                    The red dashed lines show how far off our prediction is from reality. We square these distances and average them to get the Mean Squared Error.
                  </p>
                  <p className="text-muted-foreground">
                  ...
                  <div className="space-y-8 pt-4">
                    <div className="space-y-4">
                      <div className="flex justify-between">
                        <label className="font-medium text-accent">b0</label>
                        <span className="font-mono">{b0.toFixed(0)}</span>
                      </div>
                      <Slider value={[b0]} min={tMin - (tMax - tMin)} max={tMax} step={(tMax - tMin) / 100} onValueChange={(v) => setB0(v[0])} />
                    </div>
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between">
                        <label className="font-medium text-primary">b1</label>
                        <span className="font-mono">{b1.toFixed(2)}</span>
                      </div>
                      <Slider value={[b1]} min={-500} max={500} step={1} onValueChange={(v) => setB1(v[0])} />
                    </div>
              ...
              {step === 3 && (
                <div className="space-y-6">
                  ...
                  <p className="text-muted-foreground">
                    Guessing and checking is slow. The computer uses gradients to figure out exactly which direction reduces the error.
                  </p>
          ...
          <motion.div key="step5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
            ...
            <div className="bg-card border border-border rounded-xl p-8 font-mono text-sm leading-relaxed space-y-6 overflow-x-auto">
              ...
              <div>
                ...
                <div>x_bar = ({dataset.slice(0, 3).map((r) => r[feature]).join(" + ")}) / 3 = {(featureVals.slice(0, 3).reduce((a, b) => a + b, 0) / 3).toFixed(2)}</div>
                <div>y_bar = ({dataset.slice(0, 3).map((r) => r[target]).join(" + ")}) / 3 = {(targetVals.slice(0, 3).reduce((a, b) => a + b, 0) / 3).toFixed(2)}</div>
              </div>

              <div>
                <div className="text-primary mb-2">// 3. Differences from Mean</div>
                <div>We calculate (x - x_bar) and (y - y_bar) for each row, multiply them, and sum them up.</div>
                <div className="text-muted-foreground italic mt-2">This gives the numerator of b1.</div>
              </div>

              <div>
                <div className="text-primary mb-2">// 4. Final Coefficients</div>
                <div>b1 = {actualCoeffs[1].toFixed(4)}</div>
                <div>b0 = {actualCoeffs[0].toFixed(4)}</div>
              </div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <CheckpointQuiz
                ...
                correctOptionId="B"
                nextRoute="/act/5"
                canAdvance={(worksheetScores[4] ?? 0) >= 4}
                blockedMessage="Score at least 4/5 on the worksheet to unlock the next act."
              />
            </motion.div>
          </motion.div>
        )}
      ...
      <DoubtSystem
        ...
        explanations={{
          simple: "We draw a line, measure how much it misses the dots, and keep adjusting it to miss less.",
          story: "Imagine tuning a radio dial. Turn it one way and the static gets worse. Turn it the other way and the music gets clearer. You stop when it is clearest.",
          math: "We are finding the parameters theta that minimize the cost function J(theta) = (1 / 2m) * sum((h(x) - y)^2).",
        }}
